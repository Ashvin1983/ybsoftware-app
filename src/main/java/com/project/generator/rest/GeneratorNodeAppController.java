...
@CrossOrigin(origins = "*")
public class GeneratorNodeAppController {

	@Autowired
	GeneratorNodeJsAppService generatorSprAppserv;
	
	List<String> appFileList;
	
	
	
	@RequestMapping(method=RequestMethod.POST, value="/generator/njs/app")
    public ResponseEntity<List<String>> createSpringbootProject(@RequestBody GeneratorSpringbootApp generatorspringapp) {
		System.out.println(generatorspringapp);
		appFileList=new ArrayList<String>();
		if(generatorspringapp !=null) {
		
			//appFileList=generatorSprAppserv.createNodeJsAppProject(generatorspringapp);
		}
